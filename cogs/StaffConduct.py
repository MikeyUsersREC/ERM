                        ...
                        while True:
                            if not constant_msg_data:
                                ...
                                if constant_msg_data.get('embeds'):
                                    view = EmbedCustomisation(ctx.author.id, MessageCustomisation(ctx.author.id, {"message": constant_msg_data}, persist=True, external=True), external=True)
                                else:
                                    view = MessageCustomisation(ctx.author.id, {"message": constant_msg_data}, persist=True, external=True)
...
a user upon receiving a **{infraction_type_name}**.",
                                ...
                                await message.edit(
                                    content=f"{pendingEmoji} **{ctx.author.name},** please set the message you wish to send \
a user upon receiving a **{infraction_type_name}**." if not send_dmContent.get('content') else send_dmContent.get('content'),
                                    embeds=[discord.Embed.from_dict(embed) for embed in send_dmContent.get('embeds')],
                                    view=view
                                )
                            await view.wait()
                            ...
                            send_dmContent = {
                                "content": updated_message.content if updated_message.content != f"{pendingEmoji} **{ctx.author.name},** please set the message you wish to send a user upon receiving a **{infraction_type_name}**." else "",
                                "embeds": [i.to_dict() for i in updated_message.embeds]
                            }
                            yesNoValue = YesNoMenu(ctx.author.id)
                            await message.edit(
                                content=f"{pendingEmoji} **{ctx.author.name},** please confirm below that you wish to use the content shown below.\n\n{send_dmContent['content']}",
                                embeds=[discord.Embed.from_dict(i) for i in send_dmContent["embeds"]],
                                view=yesNoValue
                            )
                            ...
                            if yesNoValue.value:
                                ...
                                constant_msg_data = send_dmContent
                    case "send_message":
            ...
            await ctx.reply(f'{successEmoji} **{ctx.author.name},** I deleted your Staff Conduct configuration.')
