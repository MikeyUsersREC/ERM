...
from menus import CustomSelectMenu, SettingsSelectMenu, ChannelSelect, RoleSelect, EnableDisableMenu, CustomModalView, \
    YesNoColourMenu, MultiSelectMenu, YesNoMenu
...
class Configuration(commands.Cog):
    ...
    async def viewconfig(self, ctx):
        ...
        embed.add_field(
            ...
            .format(
                ...
                antiping_role,
                bypass_role,
                settingContents['antiping'].get('use_hierarchy') if settingContents['antiping'].get(
                    'use_hierarchy') is not None else 'True'
            ),
            inline=False
        )
        ...
        embed.add_field(
            ...
            .format(
                ...
                shift_role,
                settingContents['shift_management'].get('nickname_prefix') if settingContents['shift_management'].get(
                    'nickname_prefix') else 'None',
                quota,
                settingContents['shift_management'].get('maximum_staff') if settingContents['shift_management'].get(
                    'maximum_staff') else 'None'
        ...
        for item in [message_logging_enabled, message_logging_channel, sts_logging_enabled, sts_logging_channel,
                     priority_logging_enabled, priority_logging_channel]:
            if item in [" ", ""]:
                item = "None"
    ...
    async def changeconfig(self, ctx):
        ...
        if category == 'verification':
            ...
            if content == 'enable':
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['verification']['role'] = [role.id for role in view.value] if [role.id for role in
                                                                                               view.value] else []
            else:
                return await invis_embed(ctx,
            ...
            if content == 'enable':
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['antiping']['role'] = [role.id for role in view.value] if [role.id for role in
                                                                                           view.value] else []
                ...
                await view.wait()
                settingContents['antiping']['bypass_role'] = [role.id for role in view.value] if [role.id for role in
                                                                                                  view.value] else []
            ...
            if content == 'enable':
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['staff_management']['role'] = [role.id for role in view.value] if [role.id for role in
                                                                                                   view.value] else []
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['staff_management']['management_role'] = [role.id for role in view.value] if [role.id
                                                                                                              for role
                                                                                                              in
                                                                                                              view.value] else []
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['staff_management']['loa_role'] = [role.id for role in view.value] if [role.id for role
                                                                                                       in
                                                                                                       view.value] else []
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['staff_management']['ra_role'] = [role.id for role in view.value] if [role.id for role
                                                                                                      in
                                                                                                      view.value] else []
            ...
            if content == 'enable':
                ...
                settingContents["moderation_sync"]["kick_ban_webhook_channel"] = view.value[
                    0].id if view.value else None
            ...
            if content == 'enable':
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['shift_management']['role'] = [role.id for role in view.value] if [role.id for role in
                                                                                                   view.value] else []
            else:
                return await invis_embed(ctx,
            ...
            if content == 'color':
                ...
                content = (
                    await request_response(bot, ctx,
                                           'What prefix do you want to use for the server? (e.g. `!`)')).content
                ...
                content = (
                    await request_response(bot, ctx, 'What server code do you use for your ER:LC server?')).content
            ...
            if content == 'enable':
                ...
                await ctx.send(embed=embed, view=view)
                await view.wait()
                settingContents['game_security']['role'] = [role.id for role in view.value] if [role.id for role in
                                                                                                view.value] else []
    ...
    async def setup(self, ctx):
        ...
        welcome = discord.Embed(title="<:Setup:1035006520817090640> Which features would you like enabled?",
                                color=0xffffff)
        ...
        if settingContents['staff_management']['enabled']:
            ...
            await view.wait()
            convertedContent = view.value
            settingContents['staff_management']['role'] = [role.id for role in
                                                           convertedContent] if convertedContent else []
            ...
            await view.wait()
            convertedContent = view.value
            settingContents['staff_management']['management_role'] = [role.id for role in
                                                                      convertedContent] if convertedContent else []
            ...
            if view.value is not None:
                if view.value:
                    ...
                    await view.wait()
                    convertedContent = view.value
                    settingContents['staff_management']['loa_role'] = [role.id for role in
                                                                       convertedContent] if convertedContent else []
            ...
            if view.value is not None:
                if view.value:
                    ...
                    await view.wait()
                    convertedContent = view.value
                    settingContents['staff_management']['ra_role'] = [role.id for role in
                                                                      convertedContent] if convertedContent else []
        ...
        if settingContents['shift_management']['enabled']:
            ...
            if view.value is not None:
                if view.value:
                    ...
                    await view.wait()
                    convertedContent = view.value
                    settingContents['shift_management']['role'] = [role.id for role in
                                                                   convertedContent] if convertedContent else []
                        ...
                        if view.value == 'enable':
                            ...
                            if view.value is True:
                                ...
                                if channel:
                                    ...
                                    return await ctx.send(embed=create_invis_embed(
                            ...
                            if view.value == 'name':
                                ...
                                if view.value is True:
                                    ...
                                    roles = [role.id for role in view.value] if [role.id for role in
                                                                                 view.value] else None
                                else:
                                    roles = []
