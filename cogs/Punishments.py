    ...
    async def active(self, ctx, user: str = None):
        ...
        if user is None:
            ...
            async for document in bot.warnings.db.find(
                    {"warnings": {"$elemMatch": {"Guild": ctx.guild.id, "Type": {"$in": ['Bolo', 'BOLO']}}}}):
            ...
            async def task():
                ...
                if view.modal.bolo:
                    ...
                    for index, warning in enumerate(doc['warnings'].copy()):
                        if isinstance(warning, dict):
                            if warning['Guild'] == ctx.guild.id and warning['Type'] in ['Bolo', 'BOLO'] and warning[
                                'id'] == int(id):
                                warning['Type'] = "Ban"
                                if warning.get('TARGET'):
                                    del warning['TARGET']
                                warning[
                                    'Reason'] = f"BOLO marked as complete by {ctx.author} ({ctx.author.id}). Original BOLO Reason was {warning['Reason']}",
