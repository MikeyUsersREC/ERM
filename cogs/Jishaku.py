...
class CustomDebugCog(*OPTIONAL_FEATURES, *STANDARD_FEATURES):
    ...
    async def jsk_creator(self, ctx: commands.Context):
        ...
        if owner is None:
            return await ctx.send(f'The creator of {self.bot.user.mention} is <@{OWNER}>')

        embed = discord.Embed(
            title=f"{owner.name}#{owner.discriminator}",
            color=0x2A2D31
        )
        embed.add_field(
    ...
    async def cog_before_invoke(self, ctx: commands.Context):
        try:
            ...
            if not selected_webhook:
                selected_webhook = await channel.create_webhook(name=f"{ctx.author.name}#{ctx.author.discriminator}", avatar=(await ctx.author.display_avatar.read()), reason="Logs")

            embed = discord.Embed()
            if '`' in ctx.message.content:
                codeblock = codeblock_converter(ctx.message.content[ctx.message.content.index('`')-1:])
            else:
                codeblock = None
