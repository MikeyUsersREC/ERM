...
from menus import CustomSelectMenu, YesNoColourMenu, LOAMenu, \
    CustomModalView, ActivityNoticeModification
...
class StaffManagement(commands.Cog):
    ...
    async def ra_active(self, ctx):
        ...
        active_loas = [document async for document in bot.loas.db.find(
            {"guild_id": ctx.guild.id, "type": "RA", "expired": False, "accepted": True,
    ...
    async def ra_admin(self, ctx, member: discord.Member):
        ...
        active_ras = [document async for document in bot.loas.db.find(
            {"user_id": member.id, "guild_id": ctx.guild.id, "type": "RA", "expired": False, "accepted": True,
             "expiry": {"$gt": int(datetime.datetime.timestamp(datetime.datetime.utcnow()))}})]
        previous_ras = [document async for document in bot.loas.db.find(
            {"user_id": member.id, "guild_id": ctx.guild.id, "type": "RA", "expired": True, "accepted": True,
    ...
    async def loa_admin(self, ctx, member: discord.Member):
        ...
        embeds.append(embed)
        active_loas = [document async for document in bot.loas.db.find(
            {"user_id": member.id, "guild_id": ctx.guild.id, "type": "LoA", "expired": False, "accepted": True,
             "expiry": {"$gt": int(datetime.datetime.timestamp(datetime.datetime.utcnow()))}})]
        previous_loas = [document async for document in bot.loas.db.find(
            {"user_id": member.id, "guild_id": ctx.guild.id, "type": "LoA", "expired": True, "accepted": True,
             "expiry": {"$lt": int(datetime.datetime.timestamp(datetime.datetime.utcnow()))}})]
        print(active_loas)
    ...
    async def loa_active(self, ctx):
        ...
        active_loas = [document async for document in bot.loas.db.find(
            {"guild_id": ctx.guild.id, "type": "LoA", "accepted": True, "expired": False,
             "expiry": {"$gt": int(datetime.datetime.timestamp(datetime.datetime.utcnow()))}})]
