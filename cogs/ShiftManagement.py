...
class ShiftManagement(commands.Cog):
    ...
    async def duty_admin(self, ctx, member: discord.Member):
        ...
        doc = [
            doc
            async for doc in bot.shifts.db.find(
                {"data": {"$elemMatch": {"guild": ctx.guild.id}}}
            )
        ]
        ...
        if status == "on" or status == "break":
            ...
            if "type" in shift.keys():
                if shift["type"]:
                    ...
                    if shift_types:
                        sh_typelist = [
                            item for item in shift_types if item["id"] == raw_shift_type
                        ]
            ...
            time_delta += (
                datetime.timedelta(seconds=sum(shift.get("added_time")))
                if shift.get("added_time") is not None
                else datetime.timedelta(seconds=0)
            )
            time_delta -= (
                datetime.timedelta(seconds=sum(shift.get("removed_time")))
                if shift.get("removed_time") is not None
                else datetime.timedelta(seconds=0)
            )
            ...
            if "breaks" in shift.keys():
                for item in shift["breaks"]:
                    if item["ended"]:
                        ...
                        break_seconds += (
                            datetime.datetime.utcnow().timestamp() - item["started"]
                        )
        ...
        if view.value == "on":
            if status == "on":
                ...
                for data in parent_item["data"]:
                    if (
                        shift["startTimestamp"] == data["startTimestamp"]
                        and shift["guild"] == data["guild"]
                    ):
                ...
                if shift_type:
                    ...
                    if shift_type:
                        ...
                        if configItem["shift_management"]["role"]:
                            if not isinstance(
                                configItem["shift_management"]["role"], list
                            ):
                                role = [
                                    discord.utils.get(
                                        ctx.guild.roles,
                    ...
                    if await bot.shifts.find_by_id(member.id):
                        ...
                        if "data" in shift.keys():
                            ...
                            await bot.shifts.update_by_id(
                                {"_id": member.id, "name": member.name, "data": newData}
                            )
            ...
            moderations = len(
                shift.get("moderations") if shift.get("moderations") else []
            )
            ...
            if toggle == "on":
                if "breaks" in shift.keys():
                    shift["breaks"].append(
                        {
                            "started": ctx.message.created_at.replace(
                                tzinfo=None
                            ).timestamp(),
                            "ended": None,
                        }
        ...
        if view.admin_value:
            if view.admin_value == "add":
                ...
                content = (
                    await request_response(
                        bot,
                        ctx,
                        "How much time would you like to add to the shift? (s/m/h/d)",
                    )
                ).content
            ...
            if view.admin_value == "remove":
                ...
                content = (
                    await request_response(
                        bot,
                        ctx,
                        "How much time would you like to remove from the shift? (s/m/h/d)",
                    )
                ).content
                content = content.strip()
                if content.endswith(("s", "m", "h", "d")):
            ...
            if view.admin_value == "clear":
                ...
                if "shifts" in document.keys():
                    ...
                    await bot.shift_storage.db.replace_one(
                ...
                if view.admin_value in ["add", "remove"]:
                    ...
                    if shift.get("removed_time"):
                        time_delta -= datetime.timedelta(
                            seconds=sum(shift["removed_time"])
                        )

                    if shift.get("added_time"):
                        time_delta += datetime.timedelta(
                            seconds=sum(shift["added_time"])
                        )
                        ...
                        for rl in role:
                            if rl in member.roles and rl is not None:
                                try:
                                    ...
                                    await invis_embed(
                                        ctx,
                                        f"Could not remove {rl} from {member.mention}",
                                    )
    ...
    async def duty_active(self, ctx):
        ...
        if not shift_type:
                    ...
                    for s in shift["data"]:
                        if s["guild"] == ctx.guild.id:
                            member = discord.utils.get(
                                ctx.guild.members, id=shift["_id"]
                            )
                            if member:
                                print(s)
                                time_delta = ctx.message.created_at.replace(
            ...
            async for shift in bot.shifts.db.find(
                {
                    "data": {
                        "$elemMatch": {"guild": ctx.guild.id, "type": shift_type["id"]}
                    }
                }
            ):
                if "data" in shift.keys():
            ...
            if member:
                ...
                if len((embeds[-1].description or "").splitlines()) < 16:
                    if embeds[-1].description is None:
                        embeds[
                            -1
                        ].description = f"<:ArrowRight:1035003246445596774> *All shift times will be here. To view active shifts, run `/duty active`*\n\n<:SConductTitle:1053359821308567592> **Total Shifts**\n<:ArrowRightW:1035023450592514048> **{index + 1}.** {member.mention} - {td_format(datetime.timedelta(seconds=i['total_seconds']))}\n"
                    else:
                        embeds[
                            -1
                        ].description += f"<:ArrowRightW:1035023450592514048> **{index + 1}.** {member.mention} - {td_format(datetime.timedelta(seconds=i['total_seconds']))}\n"
                        ...
                        if member not in added_staff:
                            ...
                            if len((embeds[-1].description or "").splitlines()) < 16:
                                if embeds[-1].description is None:
                                    ...
                                    embeds[
                                        -1
                                    ].description += f"<:ArrowRightW:1035023450592514048> **{index}.** {member.mention} - 0 seconds\n"
                ...
                if member not in added_staff:
                    ...
                    if len((embeds[-1].description or "").splitlines()) < 16:
                        if embeds[-1].description is None:
                            ...
                            embeds[
                                -1
                            ].description += f"<:ArrowRightW:1035023450592514048> **{index + 1}.** {member.mention} - 0 seconds\n"
