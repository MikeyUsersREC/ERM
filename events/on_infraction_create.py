...
class OnInfractionCreate(commands.Cog):
    ...
    async def on_infraction_create(self, infraction_doc):
        try:
            ...
            variables = {
                ...
                "{escalated}": "Yes" if infraction_doc.get("escalated", False) else "No",
                "{count}": str(await self.bot.db.infractions.count_documents({
                    "user_id": member.id,
                    "guild_id": guild.id,
                    "type": infraction_doc["type"]
                })),
            ...
            if roles_added or roles_removed:
                await self._update_role_changes(infraction_doc, roles_added, roles_removed)
            ...
            await self._process_additional_actions(
                infraction_config, guild, member
            )

        except Exception as e:
            logger.error(f"Error processing infraction: {e}")

    async def _process_role_add(self, add_config, guild, member, infraction_doc, roles_added):
        roles_to_add = []
        ...
        if roles_to_add:
            try:
                await member.add_roles(*roles_to_add, reason=f"Infraction {infraction_doc['type']}")
                if add_config.get("temporary") and add_config.get("duration"):
                    infraction_doc["temp_roles_added"] = [r.id for r in roles_to_add]
                    infraction_doc["temp_roles_added_expiry"] = datetime.datetime.now().timestamp() + add_config["duration"]
            except Exception as e:
                logger.error(f"Failed to add roles: {e}")

    async def _process_role_remove(self, remove_config, guild, member, infraction_doc, roles_removed):
        roles_to_remove = []
        ...
        if roles_to_remove:
            try:
                await member.remove_roles(*roles_to_remove, reason=f"Infraction {infraction_doc['type']}")
                if remove_config.get("temporary") and remove_config.get("duration"):
                    infraction_doc["temp_roles_removed"] = [r.id for r in roles_to_remove]
                    infraction_doc["temp_roles_removed_expiry"] = datetime.datetime.now().timestamp() + remove_config["duration"]
            except Exception as e:
    ...
    async def _process_notifications(self, notifications, guild, member, variables):
        ...
        if notifications.get("public", {}).get("enabled"):
            ...
            content = self.replace_variables(public_config.get("content", ""), variables)
        ...
        if config.get("remove_ingame_perms", False):
            try:
                ...
                if roblox_info and "robloxID" in roblox_info:
                    ...
                    await self.bot.prc_api.run_command(guild.id, f":unadmin {roblox_id}")
            except Exception as e:
                logger.error(f"Failed to remove in-game permissions: {e}")

        if config.get("end_shift", False):
            try:
                current_shift = await self.bot.shift_management.get_current_shift(member, guild.id)
                if current_shift:
                    await self.bot.shift_management.end_shift(current_shift['_id'], guild.id)
    ...
    async def _update_role_changes(self, infraction_doc, roles_added, roles_removed):
        if roles_added or roles_removed:
            ...
            try:
                ...
                logger.error(f"Failed to update infraction document with role changes: {e}")
