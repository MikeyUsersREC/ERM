        ...
        if 'game_security' in dataset.keys() or 'moderation_sync' in dataset.keys():
            ...
            if 'game_security' in dataset.keys():
                if 'enabled' in dataset['game_security'].keys():
                    if 'channel' in dataset['game_security'].keys() and 'webhook_channel' in dataset[
                        'game_security'].keys():
                                    ...
                                    if ' for ' not in command:
                                        ...
                                        for word in command.split(' ')[1:]:
                                            if not bot.get_command(combined.strip()):
                                                ...
                                                if isinstance(item, commands.HybridCommand) and not isinstance(item,
                                        ...
                                        print(command.split(' '))
                                        args = [i.replace('"', '') for i in command.split(' ')][
                                               [i.replace('"', '') for i in command.split(' ')].index(
                                                   invoked_command.split(' ')[-1]) + 1:]
                                        print(args)
                                        ...
                                        new_message.content = ((await get_prefix(bot, message))[-1]) + \
                                        ...
                                        if invoked_command.lower() == "duty on":
                                            ...
                                            if args:
                                                ...
                                                for arg in args:
                                                    if combined.strip() not in [
                                                        shift_type['name'].lower().strip() if isinstance(shift_type,
                                                                                                         dict) else shift_type.lower().strip()
                                                ...
                                                predetermined_shift_type = shift_types[[
                                                    shift_type['name'].lower().strip() if isinstance(shift_type,
                                                                                                     dict) else shift_type.lower().strip()
                                                    for shift_type in shift_types].index(combined.lower().strip())]
                                                ...
                                                if settings.get('shift_types'):
                                                    if predetermined_shift_type:
                                                        shift_type = [item for item in
                                                                      ...
                                                                      predetermined_shift_type['name'].lower().strip()]
                                                        if len(shift_type) == 1:
                                                            shift_type = shift_type[0]
                                                    elif len(settings['shift_types'].get('types') or []) > 1 and \
                                                            settings[
                                            ...
                                            moderation_list = shift.get('moderations') if shift.get(
                                                'moderations') else []
                                                        ...
                                                        shifts = [
                                                            {
                                                                ...
                                                                'moderations': shift[
                                                                    'moderations'] if 'moderations' in shift.keys() else [],
                                                                'type': shift[
                                                                    'type'] if 'type' in shift.keys() else None,
                                                            }
                                                        ]
                                                    else:
                                                        object = {
                            ...
                            if ":kick" in embed.description or ":ban" in embed.description:
                                ...
                                if 'Command Usage' in embed.title or 'Kick/Ban Command Usage' in embed.title:
                                    ...
                                    if command.count(',') + 1 >= 5:
                                        ...
                                        await aa_detection_channel.send(','.join(pings) if pings != [] else '',
                                                                        embed=embed)
                                    if " all" in command:
                                        embed = discord.Embed(
                                            title="<:WarningIcon:1035258528149033090> Excessive Moderations Detected",
