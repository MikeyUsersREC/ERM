    ...
    async def on_message(self, message: discord.Message):
        ...
        if kick_ban_webhook_channel is not None and message.channel.id == kick_ban_webhook_channel:
            for embed in message.embeds:
                ...
                if not reason:
                    reason = 'No reason provided'
                ctx = await bot.get_context(new_message)
                roblox_user = await get_roblox_by_username(violator_user, self.bot, ctx)
                ...
                punishment_types = await self.bot.punishment_types.get_punishment_types(message.guild.id)
                ...
                if action_type.lower() not in [item.lower() for item in actual_types]:
                    ...
                    return await ctx.send(embed=discord.Embed(
                        title="Invalid Type",
                        description="The type you provided is not valid.",
                        color=BLANK_COLOR
                    ))
                for item in actual_types:
                    safe_item = item.lower()
                    if action_type.lower() == safe_item:
                        actual_type = item
                        break
                if action_type.lower() in ["tempban", "temporary ban"]:
                    return await ctx.send(embed=discord.Embed(
                        title="Not Supported",
