                ...
                for member in guild.members:
                    if pattern.search(member.name) or pattern.search(member.display_name) or (hasattr(member, 'global_name') and member.global_name and pattern.search(member.global_name)):
                        ...
                        if not any(role in member.roles for role in exotic_roles):
                            await send_pm_with_rate_limit_handling(player.username, guild_id, guild.name, player.user_id, alert_channel)
                        break
                if not member_found:
                    await send_pm_with_rate_limit_handling(player.username, guild_id, guild.name, player.user_id, alert_channel, not_in_server=True)

    end_time = time.time()
    logging.warning(f"Event check_exotic took {end_time - initial_time} seconds")

async def send_pm_with_rate_limit_handling(username, guild_id, guild_name, user_id, alert_channel, not_in_server=False):
    command = f":pm {username} Your vehicle is whitelisted, but you do not have the required role. Please contact an admin or moderator."
    if not_in_server:
        command = f":pm {username} You are not in the server, but your vehicle is whitelisted. Please join the server to obtain the required role."

    while True:
        command_response = await bot.prc_api.run_command(guild_id, command)
        if command_response[0] == 200:
            logging.info(f"Sent PM to {username} in guild {guild_name} ({guild_id})")
            break
            ...
            logging.error(f"Failed to send PM to {username} in guild {guild_name} ({guild_id})")
            break

        if username not in pm_counter:
            pm_counter[username] = 1
        else:
            pm_counter[username] += 1

        if pm_counter[username] == 4:
            await alert_channel.send(embed=create_warning_embed(username, user_id, guild_name))
            pm_counter.pop(username)
