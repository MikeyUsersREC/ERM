...
class MCApiClient:
    ...
    async def _send_api_request(self, method: typing.Literal['GET', 'POST'], endpoint: str, guild_id: int,
                                data: dict | None = None, key: str | None = None):
        if not key:
        ...
        async with self.session.request(method, url=f"{self.base_url}{endpoint}", headers={
            ...
            if response.status == 429:
                ...
                return await self._send_api_request(method=method, endpoint=endpoint, guild_id=guild_id, data=data,
                                                    key=key)
    ...
    async def authorize(self, roblox_id: int, server_name: str, guild_id: int):
        status_code, response_json = await self._send_api_request("POST", '/Server/Auth', 0, {
            'RobloxId': roblox_id,
    ...
    async def fetch_server_logs(self, guild_id: int):
        status_code, response_json = await self._send_api_request('GET', '/Server/Commands', guild_id)
        if status_code == 200:
            return [CommandLog(
